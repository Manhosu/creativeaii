...
async def get_scraped_products(limit: int = 100, offset: int = 0, categoria: str = None, search: str = None):
    ...
    try:
        ...
        # 🚨 CORREÇÃO URGENTE: APLICAR FILTRO DE BUSCA POR TEXTO
        if search and search.strip():
            ...
            for product in all_products:
                # Buscar nos campos: nome, marca, código, descrição
                searchable_text = ' '.join([
                    product.get('nome', ''),
                    product.get('marca', ''),
                    product.get('codigo', ''),
                    product.get('descricao', ''),
                    product.get('categoria_nome', '')
                ]).lower()
            ...
            all_products = filtered
            unique_products = filtered_unique  # Atualizar produtos únicos para busca
            logger.info(f"🔍 Busca '{search}': {len(filtered_unique)} produtos únicos encontrados")
