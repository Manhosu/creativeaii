...
async def get_scraped_products(limit: int = 100, offset: int = 0, categoria: str = None, search: str = None):
    ...
    try:
        ...
        # 🚨 CORREÇÃO CRÍTICA: IMPLEMENTAR PESQUISA QUE ESTAVA FALTANDO!
        if search and search.strip():
            ...
            for product in all_products:
                # Garantir que os campos não sejam None antes de chamar .lower()
                ...
                # Buscar em nome, marca, código e descrição
            ...
            logger.info(f"🔍 Pesquisa '{search}': {len(filtered_unique_search)} produtos únicos encontrados")
        ...
        return {
            ...
            "message": f"✅ CORRIGIDO: {len(products)} produtos (de {total_products_unique} únicos) - {'com pesquisa' if search else 'sem duplicatas'}!"
        }
